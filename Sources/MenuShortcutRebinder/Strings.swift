...
enum Strings {
    ...
    static var setGesturePeekDesc: String { s("Zeigt das Befehls-Overlay, solange Du die Taste hältst; beim Loslassen schliesst es wieder. Für den schnellen Blick.", "Shows the command overlay while you hold the key; releasing closes it again. For a quick glance.", fr: "Affiche l'overlay tant que tu maintiens la touche ; relâcher le ferme. Pour un coup d'œil rapide.", es: "Muestra el overlay mientras mantienes la tecla; al soltar se cierra. Para un vistazo rápido.", it: "Mostra l'overlay finché tieni premuto il tasto; rilasciando si chiude. Per un'occhiata rapida.") }
    static var setGestureFixDesc: String { s("Öffnet das Befehls-Overlay dauerhaft und durchsuchbar. Schliessen mit Esc, dem X oder einer Befehlsauswahl.", "Opens the command overlay permanently and searchable. Close with Esc, the X, or by picking a command.", fr: "Ouvre l'overlay durablement et permet la recherche. Fermer avec Échap, le X ou en choisissant une commande.", es: "Abre el overlay de forma permanente y con búsqueda. Cierra con Esc, la X o eligiendo un comando.", it: "Apre l'overlay in modo permanente e ricercabile. Chiudi con Esc, la X o scegliendo un comando.") }
    ...
    static func cmdUnavailable(_ title: String) -> String { s("Gerade nicht verfügbar: \(title)", "Not available right now: \(title)", fr: "Indisponible pour l'instant : \(title)", es: "No disponible ahora: \(title)", it: "Non disponibile ora: \(title)") }
    static var browseLargeSubmenu: String { s("grosses Untermenü", "large submenu", fr: "grand sous-menu", es: "submenú grande", it: "sottomenu grande") }
